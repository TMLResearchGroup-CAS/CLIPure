...
def interpolate_state_dict(m1, beta=0.):
	m = {}
	try:
		m2 = torch.load("/mnt/nsingh/project_multimodal/models/clip-vit-b-visual.pt", map_location='cpu')
	except:  
		m2 = torch.load("/data/naman_deep_singh/project_multimodal/clip-vit-b-visual.pt", map_location='cpu')
	for k in m1.keys():
...
def load_clip_model(clip_model_name, pretrained, beta=0.):
	try:  # try loading only visual model
		...
		model, _, image_processor = open_clip.create_model_and_transforms(
			clip_model_name, pretrained=pretrained, device='cpu', precision='fp16'
		)
		...
		model, _, image_processor = open_clip.create_model_and_transforms(
			clip_model_name, pretrained=pretrained, force_quick_gelu=True, device='cpu', precision='fp16'
		)
	model.eval()
...
def compute_accuracy_no_dataloader(model, data, targets, device, batch_size=1000):
	...
	for batch_idx in range(n_batches):
		...
		# print(data_batch.requires_grad)
